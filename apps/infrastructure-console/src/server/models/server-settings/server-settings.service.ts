...
export class ServerSettingsService {
  constructor(
    ...
    private readonly icSettingsRepository: Repository<ServerSettings>,
  ) {}
  ...
  async save(params) {
    ...
    return await this.icSettingsRepository.save(serverSettings);
  }

  async find(): Promise<ServerSettings> {
    const settings = await this.icSettingsRepository.find();
    return settings.length ? settings[0] : null;
  }

  async findOne(params) {
    return await this.icSettingsRepository.findOne(params);
  }

  async update(query, params) {
    return await this.icSettingsRepository.update(query, params);
  }

  async count() {
    return this.icSettingsRepository.count();
  }
}
