...
export class SettingsComponent implements OnInit {
  ...
  authSettingsForm = new FormGroup({
    ...
    communicationServerClientId: new FormControl(this.communicationServerClientId),
  });
  ...
  ngOnInit() {
    this.settingsService.getSettings()
      .subscribe({
        next: (response: { issuerUrl: string; communicationServerClientId: string }) => {
  ...
  populateForm(response) {
    ...
    this.authSettingsForm.controls.communicationServerClientId.setValue(response.communicationServerClientId);
  }
